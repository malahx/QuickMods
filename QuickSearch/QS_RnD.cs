...
namespace QuickSearch
{
    public partial class QRnD
    {
        ...
        protected override void OnGUI()
        {
            ...
            searched = searched && (lastSearchString == Text);
            if ((lastSearchString != Text))
            {
            ...
            {
                if (!searched && Time.realtimeSinceStartup - lastTimeValueChanged > QSettings.Instance.timeToWaitBeforeSearch)
                {
                    if (Text != QSearch.Text)
                    {
                        QSearch.Text = Text;
                        searched = true;
            ...
            else
            {
                if (GUI.GetNameOfFocusedControl() == "searchField")
                {
                    if (QSettings.Instance.enableEnterToSearch && Event.current.keyCode == KeyCode.Return)
                    {
                        QSearch.Text = Text;
                        ShowHistory();
                    }
            ...
            if (!QSettings.Instance.enableEnterToSearch && Text != "")
            {
                if (GUI.GetNameOfFocusedControl() == "searchField")
                {
                    ShowHistory();
